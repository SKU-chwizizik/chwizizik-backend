...
public class GeminiService {
    ...
    public GeminiService(QuestionRepository questionRepository, InterviewRepository interviewRepository) {
    ...
    // 박 부장 프롬프트
    private final String BASIC_PROMPT = 
    ...
    // 김 팀장 프롬프트
    private final String JOB_PROMPT = 
    ...
    // 김 팀장과 박 부장 첫 질문(랜덤임 추가하려면 추가 가능)
    public String getInitialQuestion(String type) {
        Random random = new Random();
        if ("job".equals(type)) {
            List<String> jobInits = List.of(
            ...
            return jobInits.get(random.nextInt(jobInits.size()));
        }
        
        List<String> basicInits = List.of(
        ...
        return basicInits.get(random.nextInt(basicInits.size()));
    }

    @Transactional
    public Map<String, Object> getNextQuestion(Long interviewId, String userResponse, String lastQuestion, String type) {
        String selectedPrompt = "job".equals(type) ? JOB_PROMPT : BASIC_PROMPT;     // 면접 유형에 따라 프롬프트 선택

        Map<String, Object> requestBody = Map.of(
            "contents", List.of(
                Map.of("parts", List.of(Map.of("text", selectedPrompt + "\n면접관의 이전 질문: " + lastQuestion + "\n지원자 답변: " + userResponse)))
            )
        );
        ...
        try {
            ...
            if (response != null && response.containsKey("candidates")) {
                ...
                nextQuestion = (String) firstPart.get("text");
            }
        } catch (Exception e) {
        ...
        // 유저가 한 대화를 Question 엔티티로 저장
        Question questionRecord = Question.builder()
                .interview(interview)
